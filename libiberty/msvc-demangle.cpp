...
extern "C" char *msvc_demangle(const char *sym, int options) {
  auto mangled = sym;
  ...
  auto flags = llvm::MSDemangleFlags(llvm::MSDF_NoAccessSpecifier |
  ...
  if ((options & (DMGL_RET_DROP | DMGL_AUTO))) {
    ...
    auto demangled = llvm::microsoftDemangle(mangled, nullptr, nullptr, (llvm::MSDemangleFlags)flags);
  #elif LLVM_VERSION_MAJOR > 10
    auto demangled = llvm::microsoftDemangle(mangled, nullptr, nullptr, nullptr, nullptr, (llvm::MSDemangleFlags)flags);
  #else
    auto demangled = llvm::microsoftDemangle(mangled, nullptr, nullptr, nullptr, (llvm::MSDemangleFlags)flags);
  #endif
  ...
  result =
      std::regex_replace(result, std::regex("(class|struct|union|enum) "), "");
